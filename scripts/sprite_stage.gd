...
var pulse := 0.0
var breath := 0.0
var player_base_y := PLAYER_POS.y

const PLAYER_POS := Vector2(690, 420)
const ENEMY_POS := Vector2(940, 425)
const PLAYER_SHADOW := Vector2(690, 545)
const ENEMY_SHADOW := Vector2(940, 545)
...
func _ready() -> void:
	z_index = 8
	player_sprite = _make_sprite("res://assets/player_ink.svg", PLAYER_POS, 0.46)
	enemy_sprite = _make_sprite("res://assets/wolf_ink.svg", ENEMY_POS, 0.34)
	_use_monk_artwork(player_sprite)
	add_child(player_sprite)
	add_child(enemy_sprite)
	queue_redraw()

func _use_monk_artwork(sprite: Sprite2D) -> void:
	# 手绘修士立绘：纯白底水墨图，乘法混合可自然融入山水；缺资产时保留 SVG。
	var tex := load("res://assets/monk_sitting.png") as Texture2D
	if tex == null: return
	sprite.texture = tex
	var s := 340.0 / float(tex.get_height())
	sprite.scale = Vector2(s, s)
	var mat := CanvasItemMaterial.new(); mat.blend_mode = CanvasItemMaterial.BLEND_MODE_MUL
	sprite.material = mat
	player_base_y = PLAYER_SHADOW.y - 170.0
	sprite.position = Vector2(PLAYER_POS.x, player_base_y)
...
func _process(delta: float) -> void:
	...
	breath += delta
	if player_sprite:
		player_sprite.position.y = player_base_y + sin(pulse * 1.35) * 3.0
	if enemy_sprite:
...
func _draw() -> void:
	...
	_draw_shadow(PLAYER_SHADOW, Vector2(84, 17), Color(INK, 0.15))
	_draw_shadow(ENEMY_SHADOW, Vector2(66, 14), Color(INK, 0.14))

	_draw_cultivation_circle(PLAYER_POS + Vector2(0, 118), 66.0, TEAL, 0.12)
	_draw_cultivation_circle(ENEMY_POS + Vector2(0, 112), 54.0, EARTH, 0.10)
...
func _draw_player_mark() -> void:
	var p := player_sprite.position + Vector2(-62, -150)
